...
cuResizeKer = module.get_function("cuResize_xyz")
...
if __name__ == "__main__":
    ...
    img_batch = np.tile(cv2.resize(cv2.imread("trump.jpg"),
                                #    (2,2)),
                                   (2560,1080)),
                        [batch,1,1,1])
    img_batch[-1] = np.tile(cv2.resize(cv2.imread("rgba.png"),(2560,1080)),[1,1,1])
    cv2.imwrite("input.jpg", cp.asnumpy(img_batch[0]))
    output_array, _ = main(img_batch, (192,200))

    # img_batch = cp.arange(1*2*1024*3, dtype=cp.uint8).reshape((1,2,1024,3))
    # output_array, _ = main(img_batch, (1 ,1024))

    # img_batch = cp.arange(1*4*4*3, dtype=cp.uint8).reshape((1,4,4,3))
    # img_batch = cp.tile(img_batch, (1,1,1,1))
    # output_array, _ = main(img_batch, (200,200))
    print(output_array)
